<template>
  ...
  export default {
    ...
    setup() {
      ...
      const offsetLeft = ref(400);
      const cardWidth = ref(100);
      const cardHeight = ref(150);
      const gap = ref(40);
      ...
      const init = async () => {
        ...
        offsetTop.value = height -400;
        offsetLeft.value = width - 1000;
  
        // gsap.set("#pagination", {
        //   top: offsetTop.value + 330,
        //   left: offsetLeft.value*0.5,
        //   y: 200,
        //   opacity: 0,
        //   zIndex: 60
        // });
    ...
    const step = () => {
      return new Promise((resolve) => {
        ...
        gsap.to(getCard(prv), { scale: 1, ease: ease.value });
...
.card {
  ...
  box-shadow: 2px 2px 10px 2px(rgba(0, 255, 255, 0.5));
}
