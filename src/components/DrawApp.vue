...
const draw = (event: MouseEvent) => {
  if (ctx.value) {
    const x = Math.floor(event.offsetX / pixelSize.value) * pixelSize.value;
    const y = Math.floor(event.offsetY / pixelSize.value) * pixelSize.value;
    ctx.value.fillStyle = ctx.value.strokeStyle;
    ctx.value.fillRect(x, y, pixelSize.value, pixelSize.value);
  }
};
